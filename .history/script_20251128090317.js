...
handleAnswer(isCorrect, btnElement) {
  ...
  nextQuestion() {
    // 1. Unlock state: Allow options to be clicked on the next question.
    ...
    // No need for 'else' as the finish is handled in handleAnswer.
}
    showQuizResults() {
    ...
    // *** FIX: Ensure the lock state is removed when results are shown ***
    nextBtn.classList.remove('active-answer'); 

    // Hide the Next Question button and remove its display space
    nextBtn.classList.add('hidden');
