class VocabularyApp {
    ...
    toggleControlPanel(show) {
        ...
        if (show) {
            // BACK TO MENU
            document.body.classList.remove('study-mode'); // Turn off full screen
            panel.classList.remove('hidden');
            studyArea.classList.add('hidden');
        } else {
            // ENTER STUDY MODE
            document.body.classList.add('study-mode'); // Turn on full screen
    ...
    getMultilingualHtml(w) {
        ...
        this.selectedLanguages.forEach(lang => {
            if (w[lang] && langMap[lang]) html += `<div class="meaning-item"><span class="meaning-label">${langMap[lang]}:</span> ${w[lang]}</div>`;
        });
        html += '</div>';
        return html;
    }
    ...
    formatAllMeanings(w) {
        ...
        let meanings = [];
        this.selectedLanguages.forEach(lang => {
            if (w[lang] && langMap[lang]) meanings.push(`${langMap[lang]}: ${w[lang]}`);
        });
