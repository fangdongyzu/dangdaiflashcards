class VocabularyApp {
    ...
    async loadBookData(bookId) {
        ...
        try {
            ...
            const response = await fetch(`${bookId}.json`);
            if (!response.ok) throw new Error("File not found");
            const data = await response.json();
            
            // Flatten data if structure is { "L1": [...], "L2": [...] } or similar
            // Adapting based on likely structure. If it's an array of objects:
    ...
    populateLessons(data) {
        ...
        // Assuming data keys are Lesson IDs (L1, L2) or data has a lesson property
        // Strategy: Get unique lessons
        let lessons = [];
        
        // Check if data is Array or Object
        if (Array.isArray(data)) {
            // Extract unique lessons from array
        ...
        // Sort naturally (L1, L2, L10 instead of L1, L10, L2)
        lessons.sort((a, b) => {
        ...
        lessons.forEach(lesson => {
            ...
            option.value = lesson;
            option.textContent = lesson; // or formatting like "Lesson 1"
    ...
    switchTab(tabId) {
        ...
        // Update active button (might trigger multiple if multiple buttons point to same tab, simple fix:)
        const activeBtn = document.querySelector(`button[data-tab="${tabId}"]`);
        if(activeBtn) activeBtn.classList.add('active');

        // Initialize specific tab logic
        this.filterVocabulary(); // Ensure vocabulary is fresh
    ...
    renderVocabularyList() {
        ...
        this.vocabulary.forEach(word => {
            ...
            // data-label attributes are used for Mobile CSS response
            tr.innerHTML = `
    ...
    toggleDifficult() {
        const word = this.vocabulary[this.currentFlashcardIndex];
        // Check if exists based on Chinese character
    ...
    startQuiz() {
        ...
        // Limit to 20 questions if list is huge
        ...
        document.getElementById('quiz-options').style.pointerEvents = 'auto'; // Re-enable clicks
