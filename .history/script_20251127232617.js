class VocabularyApp {
    ...
    populateLessons(data) {
        ...
        // FIX: Extract unique lesson names from the flattened array of objects
        // Use a Set to get unique lesson strings, then convert back to an array
        let lessons = [...new Set(data.map(item => item.lesson).filter(lesson => lesson && lesson.trim() !== ''))];
        
        
        // Custom sort for lessons like L1, L2, L10, L1-1, L1-2, L2-1
        lessons.sort((a, b) => {
            const splitA = a.match(/(\d+)/g) || [];
            const splitB = b.match(/(\d+)/g) || [];
            
            for (let i = 0; i < Math.max(splitA.length, splitB.length); i++) {
                const numA = parseInt(splitA[i] || '0', 10);
                const numB = parseInt(splitB[i] || '0', 10);

                if (numA !== numB) {
                    return numA - numB;
                }
            }
            // Fallback to alphabetical if numbers are identical (e.g., L1-1 vs L1-2)
            if (a < b) return -1;
            if (a > b) return 1;
            return 0;
        });
