class VocabularyApp {
    ...
    selectQuizType(type) {
        ...
        if (this.quizQuestions.length === 0) {
            this.showMessage(`Not enough vocabulary to generate ${type} quiz questions.`, 'warning');
            return;
        }
    ...
    displayQuizQuestion() {
        ...
        if (this.quizQuestions.length === 0) {
            quizContent.innerHTML = `
                <div class="text-center">
                    ...
                    <button class="btn primary mt-10" onclick="app.showQuizTypeSelection()">Choose Different Quiz Type</button>
                </div>
            `;
            return;
        }
        ...
        quizContent.innerHTML = `
            <div class="quiz-container">
                <div class="quiz-question" id="quiz-question">${question.question}</div>
                ...
                <div class="progress-info">
                    <div class="progress-text" id="quiz-progress-text">Question ${this.currentQuizIndex + 1} of ${this.quizQuestions.length}</div>
                    <div class="progress-bar">
                        <div class="progress" id="quiz-progress" style="width: ${((this.currentQuizIndex + 1) / this.quizQuestions.length) * 100}%"></div>
                    </div>
                </div>
            </div>
        `;
...
// Make app globally accessible for the onclick handler
let app;
