class VocabularyApp {
    ...
    renderQuestion() {
        ...
        nextBtn.textContent = 'Next Question'; // Reset text
    ...
    <div style="font-size:1rem; line-height:1.4;">
        ...
        if (this.quizMode === 'chinese-meaning') {
            ...
            questionText = `<span class="pinyin-text" style="font-size:2.5rem">${questionData.pinyin}</span>`;
            answerType = 'chinese';
        }
...
handleAnswer(isCorrect, btnElement) {
    ...
    if (nextBtn.classList.contains('active-answer')) {
        return;
    }
    nextBtn.classList.add('active-answer'); // Set lock
    ...
    if (isCorrect) {
        ...
        // ... (Logic to highlight correct answer)
        const questionData = this.quizQuestions[this.currentQuizIndex];
        allOptions.forEach(opt => {
            if (opt !== btnElement) {
                // Simplified comparison logic for demonstration
                const optData = this.quizQuestions.find(q => {
                    // Check for a match based on the expected answer text/html
                    if (this.quizMode === 'chinese-meaning') {
                        return this.formatAllMeaningsForComparison(q) === this.formatAllMeaningsForComparison(questionData);
                    } else if (this.quizMode === 'meaning-chinese' || this.quizMode === 'pinyin-chinese') {
                        return q.chinese === questionData.chinese;
                    } else if (this.quizMode === 'chinese-pinyin') {
                        return q.pinyin === questionData.pinyin;
                    }
                    return false;
                });

                if (optData && opt !== btnElement) {
                    // Check if this option corresponds to the correct answer
                    // Note: This relies on the options being created based on unique vocabulary entries.
                    // A safer implementation would use data attributes on the options.
                    // Given the current structure, we assume the correct option is uniquely identifiable.
                    const isCorrectOptionText = (text) => {
                        if (this.quizMode === 'chinese-meaning') return opt.innerHTML.includes(this.formatAllMeaningsForComparison(questionData));
                        if (this.quizMode === 'meaning-chinese' || this.quizMode === 'pinyin-chinese') return opt.textContent === questionData.chinese;
                        if (this.quizMode === 'chinese-pinyin') return opt.textContent === questionData.pinyin;
                        return false;
                    }
                    if (isCorrectOptionText(opt)) {
    ...
    // Show 'Next Question' button only if there are more questions.
    if (this.currentQuizIndex < this.quizQuestions.length - 1) {
        nextBtn.classList.remove('hidden'); // Show the Next Question button
    } else {
        // Last question finished: wait a moment, then show results
        setTimeout(() => {
            // *** FIX: Remove the lock state before showing results ***
            nextBtn.classList.remove('active-answer');
   ...
   nextQuestion() {
        // Clear the active-answer flag
        document.getElementById('next-question-btn').classList.remove('active-answer');
        
        // Hide the button again for the next question setup
        ...
        // The check for the end of the quiz is already handled in handleAnswer 
        // by calling showQuizResults. This only proceeds to the next question.
        if (this.currentQuizIndex < this.quizQuestions.length ) {
            this.renderQuestion();
        } 
        // If currentQuizIndex is equal to quizQuestions.length, we should 
        // have already called showQuizResults from handleAnswer on the last question.
    }
    showQuizResults() {
        // Hide quiz elements (Question and Options)
        ...
        // --- VITAL CHANGE: Hide the Next Question button and remove its display space ---
        const nextBtn = document.getElementById('next-question-btn');
        nextBtn.classList.add('hidden');
        nextBtn.style.display = 'none'; // Ensure it takes up no space
