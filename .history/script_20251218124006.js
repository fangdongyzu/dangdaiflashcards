...
```javascript
    bindEvents() {
        ...
        // --- KEYBOARD & PRESENTATION REMOTE NAVIGATION ---
        let keyClickCount = 0;
        let keyTimer = null;
        ...
        document.addEventListener('keydown', (e) => {
            ...
            // Define keys for Next and Previous
            // Remotes usually send PageDown/PageUp. We also include Arrows and Space.
            const isNext = e.key === 'ArrowRight' || e.key === 'PageDown' || e.key === ' ';
            const isPrev = e.key === 'ArrowLeft' || e.key === 'PageUp';

            // Keep original Up/Down flip logic (Instant)
            ...
            if (isNext || isPrev) {
                ...
                if (keyClickCount === 1) {
                    ...
                    keyTimer = setTimeout(() => {
                        // If timer finishes, it was a Single Click
                        if (isNext) this.nextFlashcard();
                        if (isPrev) this.prevFlashcard();
                        keyClickCount = 0;
                    }, 300); // 300ms delay to wait for second click
                } else {
                    // Second click happened before timer finished -> Double Click
    ...
    handleAnswer(isCorrect, btnElement) {
        ...
        if (isCorrect) {
            ...
            this.incorrectWords.push(this.quizQuestions[this.currentQuizIndex]);
