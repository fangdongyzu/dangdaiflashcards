...
handleAnswer(isCorrect, btnElement) {
    ...
    // 1. Lock check: If locked, exit immediately.
    if (nextBtn.classList.contains('active-answer')) {
        return;
    }
    // 2. Set lock: Mark that an answer has been selected.
    nextBtn.classList.add('active-answer'); 
    ...
    // ... (Quiz scoring and visual feedback logic remains the same) ...
    ...
    if (this.currentQuizIndex < this.quizQuestions.length - 1) {
        ...
        setTimeout(() => {
            ...
            // *** Removed lock removal here, moved to showQuizResults for simplicity ***
        }, 1000); 
    }
}
