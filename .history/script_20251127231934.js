class VocabularyApp {
    ...
    // Initialize UI state
    this.disableControls(); // Add this line
    ...
    try {
        // Try JSON first (as in current code)
        const response = await fetch(`${bookId}.json`);
        if (!response.ok) {
            // If JSON fails, try CSV
            const csvResponse = await fetch(`${bookId}.csv`);
            if (!csvResponse.ok) throw new Error("Neither JSON nor CSV file found");
            
            const csvText = await csvResponse.text();
            this.fullBookData = this.parseCSV(csvText);
        } else {
            this.fullBookData = await response.json();
        }
        
        this.populateLessons(this.fullBookData);
    } catch (error) {
        console.error('Error loading book:', error);
        lessonSelect.innerHTML = '<option value="">Error loading book</option>';
        alert(`Could not load book data: ${error.message}`);
    }
}
...
disableControls() {
    const lessonSelect = document.getElementById('lesson-select');
    lessonSelect.innerHTML = '<option value="">Select Book First</option>';
    lessonSelect.disabled = true;
